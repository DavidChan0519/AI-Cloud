...
int main(int argc, char *argv[])
{
  ...
  if(curl) {
    /* First set the URL that is about to receive our POST. This URL can
       ...
       data. */
    curl_easy_setopt(curl, CURLOPT_URL, "https://api-cn.faceplusplus.com/humanbodypp/v1/segment");
    ...
    string encodeStr = res_data["result"].asString();
    string decodeStr = base64_decode(encodeStr);

    vector<char> pic(decodeStr.begin(), decodeStr.end());

    cv::Mat img2nd = cv::imdecode(pic, CV_LOAD_IMAGE_COLOR);
